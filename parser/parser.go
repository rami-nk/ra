...
func (p *Parser) parseLetStatement() *ast.LetStatement {
	...
	if !p.expectPeekTokenAndAdvance(token.IDENT) {
		return nil
	}

	stmt.Name = &ast.Identifier{Token: p.curToken, Value: p.curToken.Literal}

	if !p.expectPeekTokenAndAdvance(token.ASSIGN) {
		return nil
	}
...
func (p *Parser) expectPeekTokenAndAdvance(t token.TokenType) bool {
	if p.peekTokenIs(t) {
		p.nextToken()
		return true
	} else {
...
func (p *Parser) parseGroupedExpression() ast.Expression {
	...
	if !p.expectPeekTokenAndAdvance(token.RPAREN) {
		return nil
	}
...
func (p *Parser) parseIndexExpression(left ast.Expression) ast.Expression {
	...
	if !p.expectPeekTokenAndAdvance(token.RBRACKET) {
		return nil
	}
...
func (p *Parser) parseIfExpression() ast.Expression {
	...
	if !p.expectPeekTokenAndAdvance(token.LPAREN) {
		return nil
	}
	...
	if !p.expectPeekTokenAndAdvance(token.LBRACE) {
		return nil
	}
...
func (p *Parser) parseForExpression() ast.Expression {
	...
	iterator := p.parseIdentifier().(*ast.Identifier)
	expression.Iterator = iterator

	if !p.expectPeekTokenAndAdvance(token.IN) {
		return nil
	}
...
func (p *Parser) parseExpressionList(end token.TokenType) []ast.Expression {
	...
	if !p.expectPeekTokenAndAdvance(end) {
		return nil
	}
